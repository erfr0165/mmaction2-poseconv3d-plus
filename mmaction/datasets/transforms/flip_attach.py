...
# ATTACH dataset distinguishes actions by the performing hand
# by fipping the image the performing hand will get switched
# FlipAttach changes the label of the action accordingly
...
def flip_label(label):
    # define dictionary of action pairs (left<->right hand and vice versa)
    label_flip = {
            8: 15, 9: 16, 10: 17, 11: 18, 12: 19, 13: 20, 14: 21,
            15: 8, 16: 9, 17: 10, 18: 11, 19: 12, 20: 13, 21: 14,
            28: 34, 29: 35, 30: 36, 31: 37, 32: 38, 33: 39,
            34: 28, 35: 29, 36: 30, 37: 31, 38: 32, 39: 33,
            45: 48, 46: 49, 47: 50,
            48: 45, 49: 46, 50: 47
        }
    return label_flip.get(label, label)


@TRANSFORMS.register_module()
class FlipAttach(BaseTransform):
        ...
        if not self.lazy:
            if flip:
                ...
                if 'keypoint' in results:
                    ...
                    results['keypoint'] = kp
                    # label flipping  for attach
                    label = results['label']
                    flipped_label = flip_label(label)
                    results['label'] = flipped_label
